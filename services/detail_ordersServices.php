...
<?php class DetailOrderServices
{
  ...
  {
    $query = "SELECT * FROM detail_orders, orders, product WHERE detail_orders.order_id = {$id} and detail_orders.order_id = orders.id and detail_orders.product_id = product.id";
    $result = $this->db->select($query);
    return $result;
  }
